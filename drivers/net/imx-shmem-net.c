...
struct imx_shm_net {
	...
	spinlock_t mu_lock;	/* protect MU */
	struct clk *mu_clk;
	u32 remote_message;
	enum e_mess_state message_state;
	u32 mub_partition;
...
static void imx_shm_net_notify_tx(struct imx_shm_net *in, unsigned int num)
{
	...
	if (vring_need_event(evt, new, old)) {
		spin_lock_irqsave(&in->mu_lock, flags);
		if (MU_SendMessageTimeout2(in->mu_base, 1, in->regs.lstate,
					   IMX_SHM_NET_MU_TIMEOUT))
			dev_err(&in->pdev->dev, "%s TIMEOUT!\n", __func__);

		spin_unlock_irqrestore(&in->mu_lock, flags);
...
static void imx_shm_net_set_state(struct imx_shm_net *in, u32 state)
{
	...
	if (MU_SendMessageTimeout2(in->mu_base, 1, state,
				   IMX_SHM_NET_MU_TIMEOUT))
		dev_err(&in->pdev->dev, "%s TIMEOUT!\n", __func__);
...
static irqreturn_t imx_shm_net_int(int irq, void *data)
{
	...
	/* check irq and read mailbox */
	irqs = MU_ReadStatus(in->mu_base);

	/* check receiver register 1 is full */
	if (irqs & MU_SR_RF1_MASK1) {
		spin_lock_irqsave(&in->mu_lock, flags);
		/* get message from receive buffer */
		MU_ReceiveMsg(in->mu_base, 1, &message);
		in->remote_message = message;
				    ...
				    unsigned long event, void *group)
{
	...
	/* Ignore other irqs */
	if (*(sc_irq_group_t *)group != SC_IRQ_GROUP_REBOOTED)
		return 0;
...
/* initialize MU based on devtree info, provide the irq number */
static int imx_shm_mu_init(struct net_device *ndev, int *irq)
{
	int ret = 0;
	...
	/* set clock and irq for MU */
	np_mu = of_find_compatible_node(NULL, NULL, "fsl,imx-mu-shmem");
	if (!np_mu) {
		dev_err(&isndev->pdev->dev, "Cannot find 'imx-mu-shmem' entry in device tree\n");
		return -EINVAL;
	}
	isndev->mu_base = of_iomap(np_mu, 0);
	if (!isndev->mu_base) {
		dev_err(&isndev->pdev->dev, "Cannot find MU's base address from 'reg' in device tree\n");
		return -EINVAL;
	}

	spin_lock_init(&isndev->mu_lock);

	*irq = of_irq_get(np_mu, 0);
	if (irq == 0) {
		dev_err(&isndev->pdev->dev, "Cannot get MU's IRQ line from 'interrupts' in device tree\n");
		return -EINVAL;
	}

	isndev->mu_clk = of_clk_get(np_mu, 0);
	if (IS_ERR(isndev->mu_clk)) {
		dev_err(&isndev->pdev->dev, "Cannot MU's clock source from 'clocks' in device tree\n");
		return PTR_ERR(isndev->mu_clk);
	}

	dev_dbg(&isndev->pdev->dev, "set clock for shmem MU\n");
	ret = clk_prepare_enable(isndev->mu_clk);
	if (ret) {
		dev_err(&isndev->pdev->dev, "unable to enable MU clock\n");
		return ret;
	}

	dev_dbg(&isndev->pdev->dev, "init MU base=0x%x\n",
		((uint)isndev->mu_base));
	MU_Init(isndev->mu_base);
	MU_EnableRxFullInt(isndev->mu_base, 1);
...
static int mu_enable_reset_irq(struct net_device *ndev)
{
	...
	if (sciErr != SC_ERR_NONE) {
		dev_err(&isndev->pdev->dev, "can't obtain mu id: %d\n", sciErr);
		return sciErr;
	}
	dev_dbg(&isndev->pdev->dev, "mu ID %d\n", mu_id);

	sciErr = sc_ipc_open(&mu_ipchandle, mu_id);

	if (sciErr != SC_ERR_NONE) {
		dev_err(&isndev->pdev->dev, "can't get ipc handler: %d\n",
			sciErr);
		return sciErr;
	};

	/* Clear any pending partition reset interrupt during probe. */
	sciErr = sc_irq_status(mu_ipchandle, SC_R_MU_0A + mu_id,
			       SC_IRQ_GROUP_REBOOTED,
			&irq_status);
	if (sciErr != SC_ERR_NONE)
		dev_warn(&isndev->pdev->dev, "Cannot get partition reboot interrupt status\n");
	dev_dbg(&isndev->pdev->dev, "reset irq status = %d\n", irq_status);
	...
	register_scu_notifier(&isndev->pnotifier);
...
static int imx_shm_net_probe(struct platform_device *pdev)
{
	...
	/* initialize MU and set IRQ */
	ret = imx_shm_mu_init(ndev, &irq);
	if (ret) {
		dev_err(&in->pdev->dev, "unable to initialize MU module.\n");
		goto err_unregister;
	}
	dev_info(&in->pdev->dev, "MU is ready for cross core communication!\n");
	...
	/* we may start receiving message from this point */
	ret = request_irq(irq, imx_shm_net_int,
	...
	dev_dbg(&pdev->dev, "irq(%d) enabled\n", irq);
...
send_first:
	...
	if (MU_SendMessageTimeout2(in->mu_base, 1, in->regs.lstate,
				   IMX_SHM_NET_MU_TIMEOUT)) {
		/* sent message timeout, then EAGAIN. */
		spin_unlock_irqrestore(&in->mu_lock, flags);
		dev_dbg(&pdev->dev, "%s first message timeout!\n", __func__);
		return -EAGAIN;
	}
	spin_unlock_irqrestore(&in->mu_lock, flags);
...
err_dis_mu_clk:
	if (in->mu_clk)
		clk_disable_unprepare(in->mu_clk);

err_unregister:
	unregister_scu_notifier(&in->pnotifier);
...
static int imx_shm_net_remove(struct platform_device *pdev)
{
	...
	in->regs.lstate = IMX_SHM_NET_STATE_RESET;
	spin_lock_irqsave(&in->mu_lock, flags);
	if (MU_SendMessageTimeout2(in->mu_base, 1, IMX_SHM_NET_STATE_RESET,
				   IMX_SHM_NET_MU_TIMEOUT))
		dev_err(&in->pdev->dev, "imx_shm_net_notify_tx TIMEOUT!\n");

	spin_unlock_irqrestore(&in->mu_lock, flags);

	disable_irq(platform_get_irq(pdev, 0));
	free_irq(platform_get_irq(pdev, 0), ndev);
...
static int imx_shm_net_pm_resume(struct device *dev)
{
	...
	if (in->mu_clk) {
		ret = clk_prepare_enable(in->mu_clk);
		if (ret) {
			dev_err(&in->pdev->dev, "unable to enable mu clock\n");
			return ret;
		}
	}

	MU_Init(in->mu_base);
	MU_EnableRxFullInt(in->mu_base, 1);
